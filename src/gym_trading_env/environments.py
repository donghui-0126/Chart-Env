...
def basic_reward_function_when_execute(history : History):
    ...
    return np.log(history["portfolio_valuation", -1] / history["portfolio_valuation", -position_duration])

def sharpe_reward_function_when_execute(history: History):
    profit = np.sum(history["portfolio_cum_return", -1])
    std = np.std(history["portfolio_cum_return", -1])
    sharpe_ratio = profit / std
    return sharpe_ratio
...
class TradingEnv(gym.Env):
    ...
    def reset(self, seed = None, options=None):
        ...
        self.historical_info.set(
            ...
            execute = False,
            enter = False,
            portfolio_cum_return = np.array([]),
            asset_cum_return = np.array([])
        )
    ...
    def step(self, position_index = None):
        is_execute = (self.historical_info["position",-1] < self.positions[position_index])
        previous_price = self._get_price()
        
        
        # 현재 index의 가격으로 거래를 한다고 가정함.
        if position_index is not None: 
            self._take_action(self.positions[position_index], is_execute)
        ...
        portfolio_return_1_step =  np.log(portfolio_value / self.historical_info["portfolio_valuation", -1])
        asset_return_1_step = np.log(price / previous_price)
        
        # asset의 log return의 array를 저장
        portfolio_cum_return = np.append(self.historical_info["portfolio_cum_return", -1], portfolio_return_1_step)
        asset_buy_and_hold_cum_return = np.append(self.historical_info["asset_cum_return", -1], asset_return_1_step)
        
        done, truncated = False, False

        if portfolio_value <= self.stoploss:
            done = True
        ...
        self.historical_info.add(
            ...
            asset_cum_return = asset_buy_and_hold_cum_return
        )
